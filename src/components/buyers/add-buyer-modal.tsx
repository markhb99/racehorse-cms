...
type BuyerFormValues = z.input<typeof createBuyerSchema>
...
export function AddBuyerModal({ horseId, open, onOpenChange }: AddBuyerModalProps) {
  ...
  const onSubmit = form.handleSubmit((data) => {
    startTransition(async () => {
      const result = await createBuyer({ ...data, horse_id: horseId })
      if (result.ok) {
        toast.success('Buyer added')
        form.reset({ horse_id: horseId, status: 'pending', invoice_amount: 0, paid_amount: 0 })
        onOpenChange(false)
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <Form {...form}>
          <form id="add-buyer-form" onSubmit={onSubmit} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Jane" disabled={isPending} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Smith" disabled={isPending} {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="email" placeholder="jane@example.com" disabled={isPending} {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="+61 4xx xxx xxx" disabled={isPending} {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
        ...
        <DialogFooter>
          ...
          <Button type="submit" form="add-buyer-form" disabled={isPending}>
